...
obsidian-frontmatter-option-title = 标题
obsidian-frontmatter-option-titleTranslation = 中文标题
obsidian-frontmatter-option-aliases = 别名
obsidian-frontmatter-option-itemType = 文献类型（中英）
obsidian-frontmatter-option-date = 日期与年份
obsidian-frontmatter-option-doi = DOI
obsidian-frontmatter-option-readingStatus = 阅读状态
...
obsidian-frontmatter-option-obsidianTags = Obsidian 标签
obsidian-frontmatter-option-zoteroTags = Zotero 标签（回写入口）
obsidian-frontmatter-option-libraryId = Zotero 库 ID（兼容）
obsidian-frontmatter-option-noteVersion = 笔记版本（兼容）
...
obsidian-sync-noTombstonedNotes = 没有可恢复的已删除 Obsidian 绑定。
obsidian-sync-tombstonesRestored = 已恢复 { $count } 个墓碑笔记。
obsidian-sync-rebindResult = 已重绑定 { $rebound } 个笔记，未找到 { $unresolved } 个。
obsidian-sync-unlinkResult = 已解除 { $count } 个 Obsidian 绑定。
