...
obsidian-frontmatter-option-title = 標題
obsidian-frontmatter-option-titleTranslation = 中文標題
obsidian-frontmatter-option-aliases = 別名
obsidian-frontmatter-option-itemType = 文献类型（中英）
obsidian-frontmatter-option-date = 日期与年份
obsidian-frontmatter-option-doi = DOI
obsidian-frontmatter-option-readingStatus = 閱讀狀態
...
obsidian-frontmatter-option-obsidianTags = Obsidian 標籤
obsidian-frontmatter-option-zoteroTags = Zotero 标签（回写入口）
obsidian-frontmatter-option-libraryId = Zotero 庫 ID（相容）
obsidian-frontmatter-option-noteVersion = 筆記版本（相容）
...
obsidian-repairManagedLinks-finished = 已恢復 { $mappings } 條聯動映射、{ $records } 條同步記錄，發現 { $conflicts } 條衝突。
obsidian-sync-noTombstonedNotes = 沒有可恢復的已刪除 Obsidian 綁定。
obsidian-sync-tombstonesRestored = 已恢復 { $count } 個墓碑筆記。
obsidian-sync-rebindResult = 已重綁定 { $rebound } 個筆記，未找到 { $unresolved } 個。
obsidian-sync-unlinkResult = 已解除 { $count } 個 Obsidian 綁定。

userGuide-start-title = 歡迎使用 Obsidian Bridge！
