...
function getCandidatePaths() {
  ...
  if (process.platform === "win32") {
    candidates.push(path.join(process.env.ProgramFiles || "", "Zotero", "zotero.exe"));
    candidates.push(
...
if (configuredPath) {
  console.error(`[test-preflight] ${RUNTIME_ENV} is set, but the file does not exist:`);
  console.error(`  ${configuredPath}`);
  console.error("Update the path or unset the variable before re-running runtime tests.");
  process.exit(1);
}
...
if (discoveredPaths.length) {
  ...
  console.error("[test-preflight] Set the environment variable before running npm run test.");
  printExamples(suggestedPath);
  process.exit(1);
}

console.error("[test-preflight] No local Zotero executable was detected in common locations.");
console.error("Install Zotero 8 or point the environment variable to an existing binary.");
console.error("[test-preflight] Build-only verification remains available via npm run build.");
printExamples(suggestedPath);
process.exit(1);
