...
function accept() {
  ...
  let selected = Array.from(selection.selected) as number[];
  if (!selected.length && Number.isInteger(selection.focused)) {
    selected = [selection.focused];
  }
  args.selected = selected.map((index: number) => templateData[index]);
}
...
function initTable() {
  ...
  if (initialSelected.length) {
    requestAnimationFrame(() => {
      const firstIndex = templateData.findIndex(
        (templateName) => templateName === initialSelected[0],
      );
      if (firstIndex >= 0) {
