...
exports.setupObsidianDashboards = setupObsidianDashboards;
const hint_1 = require("../../utils/hint");
const str_1 = require("../../utils/str");
const settings_1 = require("./settings");
const frontmatter_1 = require("./frontmatter");
const paths_1 = require("./paths");
function getDashboardFilterLines(settings) {
    const conditions = [`'file.hasTag("literature")'`];
    const relativeNotesDir = (0, paths_1.getVaultRelativeFolder)(settings.vaultRoot, settings.notesDir);
    if (relativeNotesDir) {
        conditions.unshift(`'file.inFolder("${(0, paths_1.escapeForDoubleQuotedString)(relativeNotesDir)}")'`);
    }
    return conditions;
}
function getDataviewFromClause(settings) {
    const relativeNotesDir = (0, paths_1.getVaultRelativeFolder)(settings.vaultRoot, settings.notesDir);
    return relativeNotesDir ? `FROM "${relativeNotesDir}"` : "";
}
function getDataviewPagesExpression(settings) {
    const relativeNotesDir = (0, paths_1.getVaultRelativeFolder)(settings.vaultRoot, settings.notesDir);
    return relativeNotesDir
        ? `dv.pages('"${(0, paths_1.escapeForDoubleQuotedString)(relativeNotesDir)}"')`
        : "dv.pages()";
}
function getDashboardFileMarker(dashboardId, commentPrefix = "<!--", commentSuffix = "-->") {
    return `${commentPrefix} ${settings_1.DASHBOARD_MANAGED_MARKER_PREFIX} ${dashboardId} ${commentSuffix}`;
}
function buildManagedDashboardMarkdown(dashboardId, frontmatter, bodyBlocks) {
    return [
        (0, frontmatter_1.buildFrontmatter)(frontmatter),
        getDashboardFileMarker(dashboardId),
        ...bodyBlocks.map((block) => String(block || "").trim()).filter(Boolean),
    ].join("\n\n");
}
function buildDataviewBlock(body, type = "dataview") {
    return ["```" + type, body.trim(), "```"].join("\n");
}
function buildResearchDashboardMarkdown(settings) {
    const fromClause = getDataviewFromClause(settings);
    const pagesExpression = getDataviewPagesExpression(settings);
    return buildManagedDashboardMarkdown("research-dashboard", {
...
function buildTopicDashboardMarkdown(settings) {
    const fromClause = getDataviewFromClause(settings);
    return buildManagedDashboardMarkdown("topic-dashboard", {
...
function buildReadingPipelineBase(settings) {
    const filterLines = getDashboardFilterLines(settings);
    return [
        `# ${settings_1.DASHBOARD_MANAGED_MARKER_PREFIX} reading-pipeline`,
        "filters:",
        "  and:",
...
function writeManagedDashboardFile(filePath, content, marker) {
    return __awaiter(this, void 0, void 0, function* () {
        if (yield (0, str_1.fileExists)(filePath)) {
            const existing = String(yield Zotero.File.getContentsAsync(filePath, "utf-8"));
...
function ensureObsidianDashboardFiles(settings) {
    return __awaiter(this, void 0, void 0, function* () {
        const dashboardDirParent = PathUtils.parent(settings.dashboardDir);
        if (dashboardDirParent) {
            yield Zotero.File.createDirectoryIfMissingAsync(dashboardDirParent);
        }
        yield Zotero.File.createDirectoryIfMissingAsync(settings.dashboardDir);
        const files = [
            {
                path: (0, str_1.jointPath)(settings.dashboardDir, "Research Dashboard.md"),
                content: buildResearchDashboardMarkdown(settings),
                marker: getDashboardFileMarker("research-dashboard"),
            },
            {
                path: (0, str_1.jointPath)(settings.dashboardDir, "Topic Dashboard.md"),
                content: buildTopicDashboardMarkdown(settings),
                marker: getDashboardFileMarker("topic-dashboard"),
            },
            {
                path: (0, str_1.jointPath)(settings.dashboardDir, "Reading Pipeline.base"),
                content: buildReadingPipelineBase(settings),
                marker: `# ${settings_1.DASHBOARD_MANAGED_MARKER_PREFIX} reading-pipeline`,
            },
        ];
        const results = [];
        for (const file of files) {
            const status = yield writeManagedDashboardFile(file.path, file.content, file.marker);
            results.push(Object.assign(Object.assign({}, file), { status }));
        }
        return results;
    });
}
function setupObsidianDashboards() {
    return __awaiter(this, arguments, void 0, function* (options = {}) {
        var _a;
        const settings = options.settings || (yield (0, settings_1.ensureObsidianSettings)());
        const results = yield ensureObsidianDashboardFiles(settings);
        const primaryFile = ((_a = results[0]) === null || _a === void 0 ? void 0 : _a.path) || "";
        const shouldOpen = typeof options.openAfterSetup === "boolean"
            ? options.openAfterSetup
            : settings.openAfterSync;
        if (!options.quiet && primaryFile) {
            yield (0, hint_1.showHintWithLink)("Obsidian dashboards are ready.", "Show in Folder", () => {
                Zotero.File.reveal(primaryFile);
            });
        }
        if (shouldOpen && primaryFile) {
            (0, paths_1.openObsidianNote)(primaryFile);
        }
        return results;
    });
}
