...
export function setDraftMetadataFieldPlacement(
  sectionKey: MetadataSectionKey,
  fieldKey: string,
  placement: "none" | "visible" | "hidden",
) {
  for (const target of ["visible", "hidden"] as const) {
    setDraftMetadataField(sectionKey, fieldKey, target, placement === target);
  }
}
...
export function renderMetadataPresetFieldList() {
  ...
  const fieldEntries = allFieldKeys
    .filter((fieldKey) => {
      if (
        state.onlyConfigured &&
        !getMetadataPresetFieldState(state.sectionKey, fieldKey).visible &&
        !getMetadataPresetFieldState(state.sectionKey, fieldKey).hidden
      ) {
        return false;
      }
      if (!searchText) {
...
export async function duplicateObsidianMetadataPreset() {
  const library = getMetadataPresetLibrary();
  const state = ensureMetadataPresetEditorState();
  const presetName =
...
export async function deleteObsidianMetadataPreset() {
  const library = getMetadataPresetLibrary();
  const state = ensureMetadataPresetEditorState();
