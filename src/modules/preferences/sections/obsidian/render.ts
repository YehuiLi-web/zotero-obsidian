...
export function renderObsidianFrontmatterSummaryCard() {
  const summary = getPrefElement<HTMLElement>(OBSIDIAN_FRONTMATTER_SUMMARY_ID);
  if (!summary) {
    return;
  }
  ...
  summary.textContent = uiText(
    `方案：${getManagedFrontmatterPresetLabel(activePreset)} · 输出 ${selectedFields.length} 项`,
    `Preset: ${getManagedFrontmatterPresetLabel(activePreset)} · ${selectedFields.length} fields`,
  );
  summary.title = FIXED_MANAGED_FRONTMATTER_KEYS.join(", ");
}
...
export function renderObsidianMetadataSummaryCard() {
  ...
  const currentSection = metadataPresetEditorState?.sectionKey || "default";
  const visibleFields = getConfiguredFields(
    activeProfile.preset.visible,
    currentSection,
  );
  const hiddenFields = getConfiguredFields(
    activeProfile.preset.hidden,
    currentSection,
  );
  ...
  summary.textContent = uiText(
    `预设：${activeProfile.name} · 栏目：${getMetadataPresetSectionLabel(
      currentSection,
    )} · 属性 ${visibleFields.length} · 隐藏 ${hiddenFields.length} · 覆盖 ${overrideCount}`,
    `Preset: ${activeProfile.name} · Section: ${getMetadataPresetSectionLabel(
      currentSection,
    )} · Visible ${visibleFields.length} · Hidden ${hiddenFields.length} · Overrides ${overrideCount}`,
  );
}
