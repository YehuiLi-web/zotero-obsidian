...
exports.startSyncFileWatcher = startSyncFileWatcher;
exports.stopSyncFileWatcher = stopSyncFileWatcher;
exports.rememberWatchedFileState = rememberWatchedFileState;
...
function isWatcherEnabled() {
    return Boolean((0, prefs_1.getPref)("obsidian.autoSync")) && Boolean((0, prefs_1.getPref)("obsidian.watchFiles"));
}
function rememberWatchedFileState(noteId, lastModified = Date.now()) {
    addon.data.sync.watcher.knownModifiedTimes[noteId] = Math.max(Number(lastModified || 0), 0);
    delete addon.data.sync.watcher.pendingChanges[noteId];
    addon.data.sync.watcher.processing = addon.data.sync.watcher.processing.filter((processingNoteId) => processingNoteId !== noteId);
}
...
function pruneWatcherState(activeNoteIds) {
    ...
    addon.data.sync.watcher.processing = addon.data.sync.watcher.processing.filter((noteId) => activeSet.has(noteId));
}
function scanWatchedFiles() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const noteIds = yield addon.api.sync.getSyncNoteIds();
        ...
        for (const noteId of noteIds) {
            ...
            const filePath = (0, str_1.formatPath)((0, str_1.jointPath)(syncStatus.path, syncStatus.filename));
            try {
                const stat = yield IOUtils.stat(filePath);
                const currentModified = Number(stat.lastModified || 0);
                const knownModified = Math.max(Number(syncStatus.fileLastModified || 0), Number(addon.data.sync.watcher.knownModifiedTimes[noteId] || 0));
                if (!knownModified) {
        ...
        const pendingNoteIds = Object.entries(addon.data.sync.watcher.pendingChanges)
            ...
            const noteId = Number(noteIdText);
            return (now - Number(queuedAt) >= WATCH_DEBOUNCE_MS &&
        ...
        const noteItems = Zotero.Items.get(pendingNoteIds).filter((item) => Boolean((item === null || item === void 0 ? void 0 : item.isNote) && item.isNote()));
        if (!noteItems.length) {
        ...
        try {
            yield addon.hooks.onSyncing(noteItems, {
                quiet: true,
                skipActive: false,
                reason: "file-watch",
            });
            ...
            addon.data.sync.watcher.processing = addon.data.sync.watcher.processing.filter((noteId) => !pendingNoteIds.includes(noteId));
        }
    });
}
