...
[\${linkText}](\${link})`,
  },
  {
    ...
    text: `<blockquote>
\${{
  return await Zotero.${config.addonInstance}.api.convert.link2html(link, {noteItem, dryRun: _env.dryRun});
}}$
</blockquote>`,
  },
  {
  ...
  if (annotationItem.annotationComment) {
    res += await Zotero.${config.addonInstance}.api.convert.md2html(
      annotationItem.annotationComment
    );
  }
  res += await Zotero.${config.addonInstance}.api.convert.annotations2html([annotationItem], {noteItem, ignoreComment: true});
  return res;
}}$`,
  },
  {
