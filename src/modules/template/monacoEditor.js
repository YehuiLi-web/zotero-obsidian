...
exports.loadTemplateTypings = loadTemplateTypings;
exports.registerTemplateLanguage = registerTemplateLanguage;
exports.initializeTemplateEditor = initializeTemplateEditor;
 ...
 */
function loadTemplateTypings(monaco) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Load template-specific TypeScript definitions
            const response = yield Zotero.HTTP.request("GET", `chrome://${package_json_1.config.addonRef}/content/lib/js/ztypes.d.ts`);
            if (response.status === 200) {
                const tsLibPath = "ts:filename/index.d.ts";
                // Add TypeScript definitions to Monaco's JavaScript defaults
                monaco.languages.typescript.javascriptDefaults.addExtraLib(response.responseText, tsLibPath);
...
function registerTemplateLanguage(monaco) {
    try {
        ...
        const tokenizeJavaScript = (code) => {
            try {
                ...
                ztoolkit.log("JavaScript tokenization failed, using fallback highlighting");
                return null;
            }
        };
        ...
        monaco.languages.setTokensProvider("template-markdown", {
            getInitialState: () => ({
                ...
                equals: function (other) {
                    return (this.stack.length === other.stack.length &&
                        this.stack.every((item, index) => item === other.stack[index]) &&
            ...
            tokenize: (line, state) => {
                ...
                const directiveMatch = line.match(/^\/\/\s*@(use-markdown|use-refresh|beforeloop-begin|beforeloop-end|default-begin|default-end|afterloop-begin|afterloop-end)\s*$/);
                if (directiveMatch) {
                            ...
                            if (jsStart < jsEnd - 1) {
                                ...
                                if (jsCode.trim()) {
                                    ...
                                    if (jsLines && jsLines.length > 0) {
                                        ...
                                        const jsKeywords = /\b(const|let|var|function|return|if|else|for|while|do|switch|case|break|continue|try|catch|finally|throw|new|this|typeof|instanceof|in|of|true|false|null|undefined)\b/g;
                                        const jsStrings = /"([^"\\]|\\.)*"|'([^'\\]|\\.)*'|`([^`\\]|\\.)*`/g;
                                        const jsNumbers = /\b\d+(\.\d+)?\b/g;
                                        let lastIndex = jsStart;
                                        let match;
                                        // Highlight keywords
                                        while ((match = jsKeywords.exec(jsCodeForHighlight)) !== null) {
                                        ...
                                        while ((match = jsStrings.exec(jsCodeForHighlight)) !== null) {
                                            tokens.push({
                                                startIndex: jsStart + match.index,
                                        ...
                                        while ((match = jsNumbers.exec(jsCodeForHighlight)) !== null) {
                                            tokens.push({
                                                startIndex: jsStart + match.index,
        ...
        provideCompletionItems: (model, position) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (isInsideJavaScript()) {
                ...
                try {
                    ...
                    const jsCompletions = yield monaco.languages.typescript
                        .getJavaScriptWorker()
                        .then((worker) => worker(model.uri))
                        .then((client) => {
                        return client.getCompletionsAtPosition(model.uri.toString(), model.getOffsetAt(position));
                    });
                    if (jsCompletions && jsCompletions.entries) {
                        const suggestions = jsCompletions.entries.map((entry) => {
                            var _a;
                            return ({
                                label: entry.name,
                                ...
                                documentation: entry.documentation
                                    ? (_a = entry.documentation.displayParts) === null || _a === void 0 ? void 0 : _a.map((p) => p.text).join("")
                                    : "",
                ...
                const templateSuggestions = [
                    ...
                    },
                    {
                        ...
                        documentation: "Marks the beginning of code that executes before processing items",
                        detail: "Template Directive",
                    },
                    {
                        ...
                        documentation: "Marks the end of code that executes before processing items",
                        detail: "Template Directive",
                    },
                    {
                        label: "@default-begin",
                        ...
                        documentation: "Marks the beginning of the default template content for each item",
                        detail: "Template Directive",
                    },
                    {
                        label: "@default-end",
                        ...
                        documentation: "Marks the end of the default template content for each item",
                        detail: "Template Directive",
                    },
                    {
                        ...
                        documentation: "Marks the beginning of code that executes after processing all items",
                        detail: "Template Directive",
                    },
                    {
                        label: "@afterloop-end",
                        ...
                        documentation: "Marks the end of code that executes after processing all items",
                        detail: "Template Directive",
                    },
                    ...
                    },
                    {
                        label: "authors",
                        kind: monaco.languages.CompletionItemKind.Snippet,
                        insertText: '${topItem.getCreators().map((v)=>v.firstName+" "+v.lastName).join("; ")}',
                        insertTextRules: monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet,
    ...
    monaco.languages.registerHoverProvider("template-markdown", {
        provideHover: (model, position) => {
            ...
            if (directiveMatch) {
                ...
                const directiveInfo = {
                    "use-markdown": "Enables markdown processing for this template. Required for markdown syntax highlighting and rendering.",
                    "use-refresh": "Enables automatic template refresh when data changes.",
                    "beforeloop-begin": "Marks the beginning of code that executes before processing items.",
                    "beforeloop-end": "Marks the end of code that executes before processing items.",
                    "default-begin": "Marks the beginning of the default template content for each item.",
                    "default-end": "Marks the end of the default template content for each item.",
                    "afterloop-begin": "Marks the beginning of code that executes after processing all items.",
                    "afterloop-end": "Marks the end of code that executes after processing all items.",
                };
 ...
 */
function initializeTemplateEditor(monaco, editor, isDark) {
    try {
        ...
        ztoolkit.log("Failed to register template language, falling back to default:", error);
        return false;
    }
}
