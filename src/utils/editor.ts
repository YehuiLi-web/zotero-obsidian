...
function getSliceFromHTMLFallback(editor: Zotero.EditorInstance, content: string) {
  ...
  return ProseMirrorDOMParser.fromSchema(getEditorCore(editor).view.state.schema).parseSlice(
    fragment,
  );
}
...
function insert(
  ...
  if (select) {
    ...
    core.view.dispatch(
      core.view.state.tr.setSelection(TextSelection.create(core.view.state.tr.doc, to, from)),
    );
  }
}
...
function getEditorAPI(editor: Zotero.EditorInstance) {
  return (editor._iframeWindow as any).wrappedJSObject
    .ObsidianBridgeEditorAPI as EditorAPI | undefined;
}
