// Generated by zotero-plugin-scaffold
/* prettier-ignore */
/* eslint-disable */
// @ts-nocheck
export type FluentMessageId =
  | 'about-title'
  | 'alert-linkCreator-emptyNote'
  | 'alert-notValidCollectionError'
  | 'alert-notValidParentItemError'
  | 'alert-syncImportedNotes'
  | 'alert-templateEditor-shouldImport'
  | 'alert-templateEditor-unsaved'
  | 'annotation-autoAnnotation'
  | 'annotation-title'
  | 'annotationNote-enableTagSync'
  | 'annotationNote-title'
  | 'autoMDFileName'
  | 'backup'
  | 'basic-exportNotes-takeover'
  | 'basic-title'
  | 'cancel'
  | 'clearHistory'
  | 'close-shortcut'
  | 'confirm'
  | 'create'
  | 'delete'
  | 'detect'
  | 'diff-both'
  | 'diff-desc'
  | 'diff-header'
  | 'diff-md'
  | 'diff-note'
  | 'editor-expandLevel-label'
  | 'editor-keepLinks'
  | 'editor-noteLinkPreview-ctrl'
  | 'editor-noteLinkPreview-disable'
  | 'editor-noteLinkPreview-hover'
  | 'editor-noteLinkPreviewType'
  | 'editor-pinTable-label'
  | 'editor-pinTableLeft'
  | 'editor-pinTableTop'
  | 'editor-previewImage-title'
  | 'editor-resizeImage-prompt'
  | 'editor-resizeImage-title'
  | 'editor-title'
  | 'editor-toolbar-settings-copyLink'
  | 'editor-toolbar-settings-copyLinkAtSection'
  | 'editor-toolbar-settings-export'
  | 'editor-toolbar-settings-insertTemplate'
  | 'editor-toolbar-settings-openAsTab'
  | 'editor-toolbar-settings-openAsWindow'
  | 'editor-toolbar-settings-openParent'
  | 'editor-toolbar-settings-refreshSyncing'
  | 'editor-toolbar-settings-refreshTemplates'
  | 'editor-toolbar-settings-showInLibrary'
  | 'editor-toolbar-settings-updateRelatedNotes'
  | 'editor-useMagicKey'
  | 'editor-useMagicKeyShortcut'
  | 'editor-useMarkdownPaste'
  | 'embedLink'
  | 'exportDocx'
  | 'exportFreeMind'
  | 'exportMD'
  | 'exportNote'
  | 'exportPDF'
  | 'fileInterface-sync'
  | 'finish'
  | 'format'
  | 'format-blockMath'
  | 'format-blockScript'
  | 'format-bold'
  | 'format-bullet'
  | 'format-code'
  | 'format-freemind'
  | 'format-h1'
  | 'format-h2'
  | 'format-h3'
  | 'format-inlineMath'
  | 'format-inlineScript'
  | 'format-italic'
  | 'format-latex'
  | 'format-link'
  | 'format-markdown'
  | 'format-monospaced'
  | 'format-msword'
  | 'format-note'
  | 'format-numbered'
  | 'format-pdf'
  | 'format-quote'
  | 'format-strikethrough'
  | 'format-subscript'
  | 'format-superscript'
  | 'format-table'
  | 'format-textColor'
  | 'format-underline'
  | 'help'
  | 'historyPreviewTitle'
  | 'historyTitle'
  | 'importClipboard'
  | 'importNote'
  | 'inbound-step1-content'
  | 'inbound-step2-content'
  | 'inbound-step3-content'
  | 'inbound-step3-middle'
  | 'keepLink'
  | 'latex-merge'
  | 'links-embed'
  | 'links-keep'
  | 'links-remove'
  | 'links-standalone'
  | 'list-header'
  | 'list-selectAll'
  | 'markdown-autoFilename'
  | 'markdown-autoSync'
  | 'markdown-withYAMLHeader'
  | 'menu-openNoteAsBNWindow'
  | 'menuAddNote-importMD'
  | 'menuAddNote-newTemplateItemNote'
  | 'menuAddNote-newTemplateStandaloneNote'
  | 'menuEditor-copy'
  | 'menuEditor-copyLine'
  | 'menuEditor-copySection'
  | 'menuEditor-resizeImage'
  | 'menuFile-exportTemplate'
  | 'menuHelp-openUserGuide'
  | 'menuItem-openInObsidian'
  | 'menuItem-syncSelectedToObsidian'
  | 'menuTab-moveNewWindow'
  | 'menuTools-importTemplateFromClipboard'
  | 'menuTools-repairObsidianManagedLinks'
  | 'menuTools-setupObsidianDashboards'
  | 'menuTools-syncManager'
  | 'menuTools-syncSelectedToObsidian'
  | 'menuTools-templateEditor'
  | 'more'
  | 'note-inbound-header'
  | 'note-inbound-refresh'
  | 'note-inbound-sidenav'
  | 'note-outbound-header'
  | 'note-outbound-refresh'
  | 'note-outbound-sidenav'
  | 'note-preview-close'
  | 'note-preview-full'
  | 'note-preview-header'
  | 'note-preview-open'
  | 'note-preview-sidenav'
  | 'note-relation-header'
  | 'note-relation-refresh'
  | 'note-relation-sidenav'
  | 'notesTitle'
  | 'obsidian-appPath-label'
  | 'obsidian-assetsDir-label'
  | 'obsidian-autoSync'
  | 'obsidian-childNotePicker-cancel'
  | 'obsidian-childNotePicker-confirm'
  | 'obsidian-childNotePicker-help'
  | 'obsidian-childNotePicker-title'
  | 'obsidian-childNotes-defaultTags'
  | 'obsidian-childNotes-help'
  | 'obsidian-childNotes-prompt-help'
  | 'obsidian-childNotes-promptSelect'
  | 'obsidian-childNotes-tags-label'
  | 'obsidian-childNotes-title'
  | 'obsidian-contentSettings-help'
  | 'obsidian-contentSettings-title'
  | 'obsidian-dashboard-help'
  | 'obsidian-dashboardAutoSetup'
  | 'obsidian-dashboardDir-label'
  | 'obsidian-fieldLabel-JCRQ'
  | 'obsidian-fieldLabel-abstract'
  | 'obsidian-fieldLabel-abstractTranslation'
  | 'obsidian-fieldLabel-citationKey'
  | 'obsidian-fieldLabel-collection'
  | 'obsidian-fieldLabel-creators'
  | 'obsidian-fieldLabel-dateY'
  | 'obsidian-fieldLabel-datetimeAdded'
  | 'obsidian-fieldLabel-datetimeModified'
  | 'obsidian-fieldLabel-itemLink'
  | 'obsidian-fieldLabel-itemType'
  | 'obsidian-fieldLabel-itemTypeKey'
  | 'obsidian-fieldLabel-pdfLink'
  | 'obsidian-fieldLabel-qnkey'
  | 'obsidian-fieldLabel-related'
  | 'obsidian-fieldLabel-tags'
  | 'obsidian-fieldLabel-titleTranslation'
  | 'obsidian-fileNameTemplate-help'
  | 'obsidian-fileNameTemplate-label'
  | 'obsidian-frontmatter-group-library'
  | 'obsidian-frontmatter-group-links'
  | 'obsidian-frontmatter-group-reference'
  | 'obsidian-frontmatter-help'
  | 'obsidian-frontmatter-option-aliases'
  | 'obsidian-frontmatter-option-authors'
  | 'obsidian-frontmatter-option-citationKey'
  | 'obsidian-frontmatter-option-collections'
  | 'obsidian-frontmatter-option-date'
  | 'obsidian-frontmatter-option-doi'
  | 'obsidian-frontmatter-option-itemLink'
  | 'obsidian-frontmatter-option-itemType'
  | 'obsidian-frontmatter-option-libraryId'
  | 'obsidian-frontmatter-option-noteVersion'
  | 'obsidian-frontmatter-option-obsidianTags'
  | 'obsidian-frontmatter-option-pdfLink'
  | 'obsidian-frontmatter-option-publication'
  | 'obsidian-frontmatter-option-rating'
  | 'obsidian-frontmatter-option-readingStatus'
  | 'obsidian-frontmatter-option-title'
  | 'obsidian-frontmatter-option-titleTranslation'
  | 'obsidian-frontmatter-option-zoteroTags'
  | 'obsidian-frontmatter-preset-active'
  | 'obsidian-frontmatter-preset-apply'
  | 'obsidian-frontmatter-preset-custom-title'
  | 'obsidian-frontmatter-preset-dataview-description'
  | 'obsidian-frontmatter-preset-dataview-title'
  | 'obsidian-frontmatter-preset-minimal-description'
  | 'obsidian-frontmatter-preset-minimal-title'
  | 'obsidian-frontmatter-preset-recommended-description'
  | 'obsidian-frontmatter-preset-recommended-title'
  | 'obsidian-frontmatter-summary'
  | 'obsidian-frontmatter-title'
  | 'obsidian-includeAbstract'
  | 'obsidian-includeAnnotations'
  | 'obsidian-includeChildNotes'
  | 'obsidian-includeHiddenInfo'
  | 'obsidian-includeMetadata'
  | 'obsidian-itemTemplate-help'
  | 'obsidian-itemTemplate-label'
  | 'obsidian-itemTemplate-pick'
  | 'obsidian-itemType-book'
  | 'obsidian-itemType-bookSection'
  | 'obsidian-itemType-conferencePaper'
  | 'obsidian-itemType-journalArticle'
  | 'obsidian-itemType-patent'
  | 'obsidian-itemType-thesis'
  | 'obsidian-metadataPreset-active-label'
  | 'obsidian-metadataPreset-defaultName'
  | 'obsidian-metadataPreset-defaultSection'
  | 'obsidian-metadataPreset-delete'
  | 'obsidian-metadataPreset-delete-finished'
  | 'obsidian-metadataPreset-delete-lastBlocked'
  | 'obsidian-metadataPreset-help'
  | 'obsidian-metadataPreset-itemType-label'
  | 'obsidian-metadataPreset-migratedName'
  | 'obsidian-metadataPreset-name-label'
  | 'obsidian-metadataPreset-newName'
  | 'obsidian-metadataPreset-reset'
  | 'obsidian-metadataPreset-reset-finished'
  | 'obsidian-metadataPreset-resync'
  | 'obsidian-metadataPreset-save'
  | 'obsidian-metadataPreset-save-finished'
  | 'obsidian-metadataPreset-saveAs'
  | 'obsidian-metadataPreset-saveAs-finished'
  | 'obsidian-metadataPreset-search-label'
  | 'obsidian-metadataPreset-switch-finished'
  | 'obsidian-metadataPreset-title'
  | 'obsidian-metadataPreset-untitledName'
  | 'obsidian-note-abstract-title'
  | 'obsidian-note-abstractTranslation-title'
  | 'obsidian-note-emptyAbstract'
  | 'obsidian-note-emptyAbstractTranslation'
  | 'obsidian-note-hiddenInfo-title'
  | 'obsidian-note-metadata-column-key'
  | 'obsidian-note-metadata-column-value'
  | 'obsidian-note-metadata-title'
  | 'obsidian-note-tags-title'
  | 'obsidian-note-userSection-conclusion'
  | 'obsidian-note-userSection-method'
  | 'obsidian-note-userSection-notes'
  | 'obsidian-note-userSection-problem'
  | 'obsidian-note-userSection-questions'
  | 'obsidian-note-userSection-related'
  | 'obsidian-note-userSection-summary'
  | 'obsidian-note-userSection-workspace'
  | 'obsidian-noteTemplate-title'
  | 'obsidian-notesDir-label'
  | 'obsidian-open-failed'
  | 'obsidian-openAfterSync'
  | 'obsidian-outputSettings-title'
  | 'obsidian-paths-help'
  | 'obsidian-pickFile'
  | 'obsidian-pickFolder'
  | 'obsidian-repairManagedLinks'
  | 'obsidian-repairManagedLinks-finished'
  | 'obsidian-repairManagedLinks-none'
  | 'obsidian-revealAfterSync'
  | 'obsidian-setupDashboards'
  | 'obsidian-simple-help'
  | 'obsidian-sync-error'
  | 'obsidian-sync-finished'
  | 'obsidian-sync-managedNotesResynced'
  | 'obsidian-sync-missingNotesDir'
  | 'obsidian-sync-missingVaultRoot'
  | 'obsidian-sync-noItemsInScope'
  | 'obsidian-sync-noManagedNotes'
  | 'obsidian-sync-noSelection'
  | 'obsidian-sync-noTombstonedNotes'
  | 'obsidian-sync-rebindResult'
  | 'obsidian-sync-showInFolder'
  | 'obsidian-sync-tombstonesRestored'
  | 'obsidian-sync-unlinkResult'
  | 'obsidian-syncNow'
  | 'obsidian-syncScope-currentList'
  | 'obsidian-syncScope-help'
  | 'obsidian-syncScope-label'
  | 'obsidian-syncScope-library'
  | 'obsidian-syncScope-selection'
  | 'obsidian-templateSettings-title'
  | 'obsidian-title'
  | 'obsidian-translateMissingMetadata-failed'
  | 'obsidian-translateMissingMetadata-unavailable'
  | 'obsidian-vaultName-label'
  | 'obsidian-vaultRoot-label'
  | 'options'
  | 'options-Docx'
  | 'options-MD'
  | 'options-PDF'
  | 'options-linkMode'
  | 'options-mm'
  | 'options-note'
  | 'outbound-step1-content'
  | 'outbound-step2-content'
  | 'outbound-step3-content'
  | 'outbound-step3-middle'
  | 'outlinePicker-cursorLine'
  | 'pref-title'
  | 'previewContainer'
  | 'refresh'
  | 'rendered-header'
  | 'reset'
  | 'restore'
  | 'save'
  | 'saveMore'
  | 'saveOutline'
  | 'saveOutlineFreeMind'
  | 'saveOutlineImage'
  | 'saveOutlineSVG'
  | 'setAutoSync'
  | 'setOutline'
  | 'share'
  | 'skip'
  | 'snippet-dryRunFlag'
  | 'snippet-exportLatexFileContentLatexContent'
  | 'snippet-exportLatexFileContentNoteItem'
  | 'snippet-exportMDFileContentMDContent'
  | 'snippet-exportMDFileContentNoteItem'
  | 'snippet-exportMDFileHeaderNoteItem'
  | 'snippet-exportMDFileNameNoteItem'
  | 'snippet-itemAfterLoop'
  | 'snippet-itemBeforeLoop'
  | 'snippet-itemCopyNoteImage'
  | 'snippet-itemFieldAbstract'
  | 'snippet-itemFieldAuthors'
  | 'snippet-itemFieldCitKey'
  | 'snippet-itemFieldDOI'
  | 'snippet-itemFieldDOIURL'
  | 'snippet-itemFieldDate'
  | 'snippet-itemFieldJournal'
  | 'snippet-itemFieldTitle'
  | 'snippet-itemFieldTitleTranslation'
  | 'snippet-itemInLoop'
  | 'snippet-itemItem'
  | 'snippet-itemItems'
  | 'snippet-itemSharedObj'
  | 'snippet-itemTargetNoteItem'
  | 'snippet-itemTopItem'
  | 'snippet-quickImportLink'
  | 'snippet-quickImportNoteItem'
  | 'snippet-quickInsertLink'
  | 'snippet-quickInsertLinkText'
  | 'snippet-quickInsertNoteItem'
  | 'snippet-quickInsertSubNoteItem'
  | 'snippet-quickNoteAnnotationItem'
  | 'snippet-quickNoteNoteItem'
  | 'snippet-quickNoteTopItem'
  | 'snippet-textSharedObj'
  | 'snippet-textTargetNoteItem'
  | 'snippet-useMarkdown'
  | 'snippet-useRefresh'
  | 'standaloneLink'
  | 'sync'
  | 'sync-attachmentFolder-label'
  | 'sync-manager'
  | 'sync-period-label'
  | 'sync-running-hint-check'
  | 'sync-running-hint-diff'
  | 'sync-running-hint-finish'
  | 'sync-running-hint-synced'
  | 'sync-running-hint-title'
  | 'sync-running-hint-upToDate'
  | 'sync-running-hint-updateMD'
  | 'sync-running-hint-updateNote'
  | 'sync-start-hint'
  | 'sync-stop-hint'
  | 'sync-title'
  | 'syncInfo-cancel'
  | 'syncInfo-export'
  | 'syncInfo-lastSync'
  | 'syncInfo-manager'
  | 'syncInfo-reveal'
  | 'syncInfo-sync'
  | 'syncInfo-syncTo'
  | 'syncInfo-unSync'
  | 'syncManager-detectConfirmInfo'
  | 'syncManager-filePath'
  | 'syncManager-lastSync'
  | 'syncManager-noteName'
  | 'tab-inbound'
  | 'tab-outbound'
  | 'target'
  | 'template-editor'
  | 'template-title'
  | 'templateEditor-templateDisplayName'
  | 'templateEditor-templateDisplayType'
  | 'templateEditor-templateHelp'
  | 'templateEditor-templateName'
  | 'templateEditor-templateType'
  | 'templateName'
  | 'templatePicker-itemData-info'
  | 'templatePicker-itemData-title'
  | 'templatePicker-itemData-useCustom'
  | 'templatePicker-itemData-useLibrary'
  | 'templateType'
  | 'templateType-help'
  | 'templateType-item'
  | 'templateType-system'
  | 'templateType-text'
  | 'templateType-unknown'
  | 'title'
  | 'toggleOutlinePane'
  | 'unSync'
  | 'unsync'
  | 'useBubbleMap'
  | 'useDefaultExport'
  | 'useMindMap'
  | 'useTreeView'
  | 'userGuide-createNote-desc'
  | 'userGuide-createNote-next'
  | 'userGuide-createNote-title'
  | 'userGuide-createNoteButton-desc'
  | 'userGuide-createNoteButton-title'
  | 'userGuide-createNoteFound-desc'
  | 'userGuide-finish-desc'
  | 'userGuide-finish-title'
  | 'userGuide-openNote-desc'
  | 'userGuide-openNote-next'
  | 'userGuide-openNote-title'
  | 'userGuide-start-close'
  | 'userGuide-start-desc'
  | 'userGuide-start-title'
  | 'userGuide-workspace-desc'
  | 'userGuide-workspace-title'
  | 'userGuide-workspaceEditor-desc'
  | 'userGuide-workspaceEditor-title'
  | 'userGuide-workspaceEditorLinkCreator-desc'
  | 'userGuide-workspaceEditorLinkCreator-title'
  | 'userGuide-workspaceEditorMoreOptions-desc'
  | 'userGuide-workspaceEditorMoreOptions-title'
  | 'userGuide-workspaceEditorToolbar-desc'
  | 'userGuide-workspaceEditorToolbar-title'
  | 'userGuide-workspaceNoteInfo-desc'
  | 'userGuide-workspaceNoteInfo-title'
  | 'userGuide-workspaceOutline-desc'
  | 'userGuide-workspaceOutline-title'
  | 'userGuide-workspaceOutlineMode-desc'
  | 'userGuide-workspaceOutlineMode-title'
  | 'userGuide-workspaceOutlineSaveAs-desc'
  | 'userGuide-workspaceOutlineSaveAs-title'
  | 'withYAMLHeader'
  | 'workspace-emptyWorkspaceGuideCreate'
  | 'workspace-emptyWorkspaceGuideInfo'
  | 'workspace-emptyWorkspaceGuideOpen'
  | 'workspace-emptyWorkspaceGuideOr'
  | 'workspace-saveOutlineFreeMind'
  | 'workspace-saveOutlineImage'
  | 'workspace-switchOutline';
